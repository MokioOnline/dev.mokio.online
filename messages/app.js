...
let callState = { pc: null, stream: null, call: null, otherId: null };
...
async function openChat(otherId, title, existing) {
  ...
  await loadConvos();
  const msgs = await request(SUPABASE_URL + '/rest/v1/messages?conversation_id=eq.' + convo.id + '&select=*&order=created_at.asc', { headers: headers(session.access_token) });
  renderThread(msgs.data || [], title);
}
...
$('composer').onsubmit = async (e) => {
  ...
  const res = await request(SUPABASE_URL + '/rest/v1/rpc/post_message', {
    method: 'POST', headers: headers(session.access_token), body: JSON.stringify(payload)
  });
  if (!res.ok) {
    const fallback = await request(SUPABASE_URL + '/rest/v1/messages', {
      method: 'POST', headers: { ...headers(session.access_token), Prefer: 'return=minimal' }, body: JSON.stringify(payload)
    });
    if (!fallback.ok) { alert((fallback.data && fallback.data.message) || 'Could not send'); return; }
  }
  e.target.reset();
  if (currentChannel) openChannel(currentChannel);
  else openChat(current.user_a === me.id ? current.user_b : current.user_a, $('title').textContent, current);
};
...
$('newServerBtn').onclick = async () => {
  ...
  if (!res.ok) { alert((res.data && res.data.message) || 'Could not create server. Run the server SQL.'); return; }
  await loadServers();
};
...
async function addChannel(kind) {
  ...
  if (!name) return;
  const res = await request(SUPABASE_URL + '/rest/v1/rpc/add_channel', {
    method: 'POST', headers: headers(session.access_token),
    body: JSON.stringify({ sid: activeServer.id, cname: name, ckind: kind })
  });
  if (!res.ok) alert((res.data && res.data.message) || 'Could not add channel');
...
$('inviteForm').onsubmit = async (e) => {
  ...
  const res = await request(SUPABASE_URL + '/rest/v1/rpc/invite_member', {
    method: 'POST', headers: headers(session.access_token),
    body: JSON.stringify({ sid: activeServer.id, uid: other.id })
  });
  alert(res.ok ? 'Invited ' + handle : ((res.data && res.data.message) || 'Invite failed'));
  e.target.reset();
};
...
async function openChannel(ch) {
  ...
  const msgs = await request(SUPABASE_URL + '/rest/v1/rpc/channel_messages', {
    method: 'POST', headers: headers(session.access_token), body: JSON.stringify({ cid: ch.id })
  });
  const list = Array.isArray(msgs.data) ? msgs.data : [];
  renderThread(list, '# ' + ch.name);
}

async function joinVoice(ch) {
  currentChannel = ch; current = null;
  showPane('voicePane', '🔊 ' + ch.name);
  ...
  if (voice.stream) {
    ...
    v.autoplay = true; v.muted = true; v.playsInline = true; v.srcObject = voice.stream;
...
async function refreshVoicePeople() {
  ...
  (Array.isArray(res.data) ? res.data : []).forEach((p) => {
    const card = document.createElement('div');
    card.className = 'card';
    card.textContent = p.name + (p.user_id === me.id ? ' (you)' : '');
    $('voicePeople').appendChild(card);
  });
}
$('muteBtn').onclick = () => {
  if (!voice.stream) return;
  voice.stream.getAudioTracks().forEach((tr) => { tr.enabled = !tr.enabled; });
  $('muteBtn').textContent = voice.stream.getAudioTracks().some((tr) => tr.enabled) ? 'Mute' : 'Unmute';
};
$('camBtn').onclick = () => { if (voice.stream) voice.stream.getVideoTracks().forEach((tr) => { tr.enabled = !tr.enabled; }); };
...
async function attachMedia() {
  ...
  if (callState.stream) {
    const v = document.createElement('video');
    v.autoplay = true; v.muted = true; v.playsInline = true; v.srcObject = callState.stream;
    $('videos').appendChild(v);
  }
}
...
setInterval(pollCalls, 2500);
setInterval(() => { if (voice.channel) refreshVoicePeople(); }, 4000);
