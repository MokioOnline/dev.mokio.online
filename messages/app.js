...
function startRing() {
  ...
  try {
    ...
    const beep = () => {
      ...
      g.gain.setValueAtTime(0.08, ringCtx.currentTime);
      o.start();
      o.stop(ringCtx.currentTime + 0.35);
    };
    beep();
...
async function joinVoice(ch) {
  ...
  if (voice.stream) {
    ...
    v.id = 'localVid'; v.autoplay = true; v.muted = true; v.playsInline = true; v.srcObject = voice.stream;
...
function paintCallButtons() {
  ...
  if ($('muteBtn')) {
    $('muteBtn').textContent = audioOn ? 'Mic on' : 'Muted';
    $('muteBtn').classList.toggle('off', !audioOn);
  }
  if ($('camBtn')) {
    $('camBtn').textContent = videoOn ? 'Cam on' : 'Cam off';
