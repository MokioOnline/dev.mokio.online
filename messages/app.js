...
function showPane(name, title) {
  ...
  ['friendsPane','notesPane','settingsPane','chatPane','voicePane'].forEach((id) => {
    if (id === 'voicePane') $(id).hidden = !inCall || name !== 'voicePane';
    else $(id).hidden = id !== name;
  });
  if (!inCall) $('voicePane').hidden = true;
...
if ($('cogBtn')) $('cogBtn').onclick = () => showPane('settingsPane', 'Account');
